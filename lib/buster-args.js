var when = require("when");
var argsOpt = require("./option");
var argsOpd = require("./operand");
var argsShorthand = require("./shorthand");

module.exports = {
    validators: require("./validators"),

    createOption: function () {
        var option = argsOpt.create(Array.prototype.slice.call(arguments));
        var i, l;

        for (i = 0, l = this.options.length; i < l; i++) {
            if (!argsOpd.isPrototypeOf(this.options[i]) &&
                    option.intersects(this.options[i])) {
                throw new Error("Duplicate option (" + option.signature + ")");
            }
        }

        this.options.push(option);
        return option;
    },

    createOperand: function () {
        var operand = argsOpd.create();
        this.options.push(operand);
        return operand;
    },

    addShorthand: function (option, args) {
        if (!argsOpt.isOption(option)) {
            throw new Error("Invalid option '" + option + "'");
        }

        if (!(args instanceof Array)) {
            throw new Error(
                "Second argument to addShorthand needs to be an array."
            );
        }

        var i, l;

        for (i = 0, l = this.options.length; i < l; i++) {
            if (this.options[i].hasOption &&
                    this.options[i].hasOption(option)) {
                throw new Error("Can not add shorthand '" + option +
                                "', option already exists.");
            }
        }

        this.options.push(argsShorthand.create(option, args));
    },

    handle: function (argv, onFinished) {
        this.args = argv;
        this.prevArgs = null;
        this.promises = [];
        this.onFinished = onFinished;
        var i, l;

        for (i = 0, l = this.options.length; i < l; i++) {
            this.options[i].reset();
        }

        this.handleOptions();
    },

    handleOptions: function () {
        var unhandledOptions, i, l, promise;

        if (!this.argsHasChanged(this.args, this.prevArgs) ||
                this.args.length === 0) {
            if (this.args.length > 0) {
                this.onFinished(["Unknown argument '" + this.args[0] + "'."]);
            } else {
                unhandledOptions = this.getUnhandledOptions();

                for (i = 0, l = unhandledOptions.length; i < l; i++) {
                    this.promises.push(unhandledOptions[i].validatorPromise());
                }

                when.all(this.promises).then(function () {
                    this.onFinished();
                }.bind(this), function (error) {
                    this.resetAll();
                    this.onFinished([error]);
                }.bind(this));
            }
            return;
        }
        this.prevArgs = Array.prototype.slice.call(this.args);

        for (i = 0, l = this.options.length; i < l; i++) {
            if (this.args.length === 0) { break; }
            promise = this.options[i].handle(this.args);
            if (promise) {
                this.promises.push(promise);
                this.handleOptions();
                return;
            }
        }

        this.handleOptions();
    },

    getUnhandledOptions: function () {
        var i, l, unhandledOptions = [];

        for (i = 0, l = this.options.length; i < l; i++) {
            if (this.options[i].handled === false) {
                unhandledOptions.push(this.options[i]);
            }
        }

        return unhandledOptions;
    },

    resetAll: function () {
        var i, l;
        for (i = 0, l = this.options.length; i < l; i++) {
            this.options[i].reset();
        }
    },

    argsHasChanged: function (args, prevArgs) {
        var i, l;

        if (prevArgs) {
            if (args.length !== prevArgs.length) {
                return true;
            }

            for (i = 0, l = args.length; i < l; i++) {
                if (args[i] !== prevArgs[i]) {
                    return true;
                }
            }
        } else {
            return true;
        }

        return false;
    },

    get options() {
        if (!this._options) { this._options = []; }
        return this._options;
    },

    set options(value) { throw new Error("options is not writable"); }
};

var flag;
for (flag in argsOpd.flags) {
    module.exports[flag] = argsOpd.flags[flag];
}
