...
module.exports = {
    ...
    addShorthand: function (option, args) {
        ...
        for (var i = 0, ii = this.options.length; i < ii; i++) {
            if (this.options[i].hasOption(option)) {
    ...
    addMutatorsToPromises: function () {
        for (var i = 0, ii =  this.options.length; i < ii; i++) {
            var option = this.options[i];
            if ("mutator" in option) {
                this.promises.push(option.createMutatorPromise());
            }
        }
    },
